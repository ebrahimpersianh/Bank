...
/**
 * خروجی اکسل (.xlsx) لیست چک‌ها - هم‌الگو با ui/stats/StatsXlsxExporter (همون تکنیکِ ساختِ دستیِ
 * OOXML حداقلی با ZipOutputStream خودِ جاوا، بدون کتابخونه‌ی جدید).
 */
object ChequeXlsxExporter {
    fun export(cheques: List<ChequeEntity>, out: OutputStream) {
        ...
        val rows = buildList<List<Any>> {
            ...
            add(emptyList())
            add(listOf("تعداد کل", active.size))
            add(listOf("دریافتی", active.count { it.type == "RECEIVED" }))
            add(listOf("پرداختی", active.count { it.type == "PAID" }))
            add(listOf("پاس‌شده", active.count { it.status == "PASSED" }))
            add(listOf("برگشت‌خورده", active.count { it.status == "BOUNCED" }))
        }
...
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
  <sheets>
    <sheet name="گزارش چک‌ها" sheetId="1" r:id="rId1"/>
  </sheets>
</workbook>"""
