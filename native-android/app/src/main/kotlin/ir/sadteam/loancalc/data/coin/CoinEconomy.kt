...
enum class CoinSpend(
    ...
    // قیمت یک دور به ۴۰۰ آمده بود چون طراح گفت بافتِ تصویری نمی‌سازد. دورِ ۱۴ خودش پس گرفت:
    // بافت با `DrawScope` شدنی است (`ui/background/ArtTexture.kt`)، پس تم دوباره چیزی دارد که
    // تمِ رنگی ندارد و ۸۰۰ برمی‌گردد.
     ...
     *
     * قیمت بینِ تمِ رنگی (۱۵۰) و ستِ نماد (۵۰۰) نشست چون کارِ ساختش هم همان‌جاست: هر
     * پس‌زمینه یک انیمیشنِ دستیِ `Canvas` است، نه یک پالتِ سه‌رنگه؛ ولی فایلِ هنری هم
     * نمی‌خواهد. و چون **روی هر تمی** می‌نشیند، با خریدِ قبلیِ کاربر رقابت نمی‌کند -
     * تنها قلمی از این ویترین که خریدِ دوم را بی‌معنی نمی‌کند.
     */
    LIVE_BACKDROP(250, "پس‌زمینه‌ی زنده", ShopCategory.BACKDROP, timeGated = false),
...
enum class ShopCategory(val label: String, val tab: String) {
    // 🚨 **پس‌زمینه اولِ ترتیب است** (بندِ ۱ی وصله‌ی بخشِ ۷۸): نوعِ تازه است و اگر
    // ششم باشد کسی که تبِ «همه» را اسکرول نمی‌کند اصلاً نمی‌بیندش.
    BACKDROP("پس‌زمینه‌ی زنده", "پس‌زمینه"),
    THEME("تم", "تم"),
