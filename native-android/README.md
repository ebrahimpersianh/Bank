...
- ✅ **گیت ورود اجباری + حالت مهمان** (`ui/auth/`) — `AuthViewModel`/`LoginScreen` پورت
  ...
  (`AuthPrefs`) ذخیره می‌شه. هنوز نداره: صفحه‌ی تنظیمات/خروج از حساب (پس فعلاً راهی برای logout
  از اپ نیست).
...
- ⏳ ویرایش دستی مبلغ هر قسط، مدل کامل پرداخت مستقل هر قسط (با تاریخ/تاخیر)، فرم تاریخ شروع وام
  دستی، تنظیمات (و خروج از حساب)، سینک ابری واقعی، خرید واقعی اشتراک کافه‌بازار، قفل اثر انگشت،
  و تم روشن هنوز پورت نشدن (فاز بعد).
