...
- ✅ **تب «وام‌های من» (`ui/myloans/`)** — زنجیره‌ی کامل Compose → Hilt ViewModel → Repository →
  ...
  `renderTable`)، و ویرایش دستی مبلغ هر قسط (پورت `confirmEditInstallment`، بدون گزینه‌ی «همین
  مبلغ رو بقیه هم بگیرن» وب). `paidLate`/`paidDate` (تاخیر در پرداخت) هنوز پورت نشدن.
...
- ⏳ ثبت تاریخ/تاخیر پرداخت هر قسط (`paidLate`/`paidDate`)، گزینه‌ی «همین مبلغ ویرایش‌شده رو بقیه
  هم بگیرن»، و اندازه فونت/یادآوری سررسید هنوز پورت نشدن (فاز بعد).
