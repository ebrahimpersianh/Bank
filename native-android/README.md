...
- ✅ **تب «وام‌های من» (`ui/myloans/`)** — زنجیره‌ی کامل Compose → Hilt ViewModel → Repository →
  ...
  `saveManualLoan` از `www/index.html`) و حذف وام از رو کارت پیاده شده.
- ✅ **رفع باگ ارقام فارسی/عربی** — `cleanNum`/`cleanNumDecimal` (پورت از وب) به `core/` اضافه
  شد و تو ورودی‌های عددی تب «وام بانکی» وصل شده (قبلش با کیبورد فارسی مقدار صفر parse می‌شد).
- ⏳ ویرایش/پرداخت قسط، بازکردن جزئیات/جدول یه وام ذخیره‌شده (`openSavedLoan`)، فرم تاریخ شروع
  وام دستی، محدودیت «۱ وام رایگان»/اشتراک، پورت واقعی تب‌های محاسبه‌گر/سود سپرده، تنظیمات، سینک
  ابری واقعی، ورود OTP، اشتراک کافه‌بازار، قفل اثر انگشت، و تم روشن هنوز پورت نشدن (فاز بعد).
