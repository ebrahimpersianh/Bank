...
- ✅ **معماری چندماژوله** — ماژول جدید `data/` (Room برای «وام‌های من»، DataStore برای
  ...
  (`di/AppModule.kt`)، Navigation-Compose به‌جای سوییچ دستی تب‌ها تو `MainActivity`. تب
  «وام‌های من» (`ui/myloans/`) اولین جایی‌یه که این زنجیره‌ی کامل (Compose → Hilt ViewModel →
  Repository → Room) واقعاً سیم‌کشی شده — فعلاً فقط لیست خالی/موجود رو نشون می‌ده، بدون فرم
  افزودن یا سینک ابری.
- ⏳ فرم افزودن/ویرایش وام، پورت واقعی تب‌های محاسبه‌گر/سود سپرده، تنظیمات، سینک ابری واقعی،
  ورود OTP، اشتراک کافه‌بازار، قفل اثر انگشت، و تم روشن هنوز پورت نشدن (فاز ۱ به بعد).
