 ...
 * سال‌های کبیسه - نه یه چرخه‌ی ثابت ساده). عیناً همون پیاده‌سازیِ تاییدشده‌ی پروژه‌ی «وام من»
 * (native-android) تو همین ریپوئه که با ده‌ها هزار تاریخ تصادفی و چند بازه‌ی کامل روزانه (شامل
 * مرزهای اسفند ۲۹/۳۰ برای چندین سال) در برابر کتابخونه‌ی پایتون jdatetime تایید شده - نمونه‌هاش تو
 * JalaliCalendarTest.kt همین ماژول. دستکاریش نکن مگر با همون سطح تست.
 */
object JalaliCalendar {
    ...
    /** فاصله‌ی روزهای واقعی بین دو تاریخ شمسی - مستقیم از رو شماره روز ژولینی داخلی الگوریتم. */
    fun daysBetween(from: PersianDate, to: PersianDate): Int =
        j2d(to.y, to.m, to.d) - j2d(from.y, from.m, from.d)

    /** تعداد روزهای واقعی یه ماه شمسی (با در نظر گرفتن کبیسه‌ی واقعی اسفند). */
    ...
    /** روز هفته با شمارش شنبه=۰ تا جمعه=۶ (هفته‌ی ایرانی) - فرمول `(jdn + 2) mod 7` که در برابر
     * jdatetime روی چندین تاریخ مرجع تایید شده (JalaliCalendarTest). */
