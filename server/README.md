...
(`src/subscriptionStatus.js`) پیاده شده: علاوه بر `subscribed`/`subscribed_until`، اگه هنوز تو ۷ روزِ
...
هر مسیری که `isSubscribed(user)` رو صدا می‌زنه (`routes/auth.js` `/me`، `routes/loans.js` PUT) باید
...
(`routes/loans.js` PUT، ۴۰۳ اگه `isSubscribed=false` و آرایه بیش از ۱ تا) چک می‌شه، نه کلاینت — یعنی
...
چند روز به `subscribed_until` کاربر اضافه کنه (`routes/subscription.js`، ثابت `TIER_DURATION_DAYS`).
...
1. اپ (`ir.sadteam.loancalc`) رو تو پنل کافه‌بازار (Pishkhan) ثبت کنید و ۴ تا محصول درون‌برنامه‌ای
   ...
   `www/index.html` و `TIER_DURATION_DAYS` تو `routes/subscription.js` یکی باشن):
